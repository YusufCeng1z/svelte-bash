...
export class Shell {
    ...
    private customCommands: Record<string, CommandHandler>;
    private cwd: string[];
    ...
    public setHistory(h: TerminalLine[]) {
        this.history = h;
    }

    public setStructure(s: FileStructure) {
        this.structure = s;
    }

    public setCommands(c: Record<string, CommandHandler>) {
     ...
     * - Nested paths ( a/b/c )
     */
    private resolvePath(pathStr: string): { node: FileStructure | string | undefined, path: string[], parent: FileStructure | undefined, name: string } {
        // 1. Determine start point
        let parts: string[] = [];
        let currentPath: string[] = [];
        let current: FileStructure | string = this.structure;
        ...
        let parent: FileStructure | undefined = undefined;
        let lastPart = '';
    ...
    public async execute(input: string): Promise<TerminalLine[]> {
        ...
        if (this.customCommands[cmd]) {
            try {
                ...
                if (res) return [{ type: 'output', content: res as any }];
                return [];
            } catch (err: any) {
        ...
        if (cmd === 'clear') {
            ...
            return [{ type: 'command' as any, content: 'CLEAR_SIGNAL', id: -1 }];
        }
        ...
        if (cmd === 'mkdir') {
            const target = args[0];
            if (!target) return [{ type: 'error', content: 'usage: mkdir <directory>' }];
        ...
        if (cmd === 'touch') {
            const target = args[0];
            if (!target) return [{ type: 'error', content: 'usage: touch <file>' }];
        ...
        if (cmd === 'rm') {
            ...
            if (!target) return [{ type: 'error', content: 'usage: rm [-r] <file/directory>' }];
        ...
        if (cmd === 'cp') {
            const src = args[0];
            const dest = args[1];
            if (!src || !dest) return [{ type: 'error', content: 'usage: cp <source> <destination>' }];

            const sourceRes = this.resolvePath(src);
            if (!sourceRes.node) return [{ type: 'error', content: `cp: cannot stat '${src}': No such file or directory` }];
        ...
        if (cmd === 'mv') {
            const src = args[0];
            const dest = args[1];
            if (!src || !dest) return [{ type: 'error', content: 'usage: mv <source> <destination>' }];

            const sourceRes = this.resolvePath(src);
            if (!sourceRes.node) return [{ type: 'error', content: `mv: cannot stat '${src}': No such file or directory` }];
        ...
        if (isCommand) {
            ...
            if (node && typeof node === 'object') {
                const entries = Object.keys(node).map(name => {
                    ...
                    const isDir = typeof (node as any)[name] === 'object';
